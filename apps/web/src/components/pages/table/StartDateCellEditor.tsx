...
/**
 * Start-date picker for the Table view's `startDate` column.
 *
 * The shared `DueDateCellEditor` lives in `listview/` and always boots on the
 * "Due" tab (`useState<Tab>('due')`), so reusing it for a Start-Date cell opens
 * the wrong tab. We are not allowed to edit that shared component, so this is a
 * thin, self-contained editor that writes `startDate` directly — no tab to get
 * wrong. It reuses the SAME shared calendar math + Menu primitives.
 */
...
import { monthGrid, monthLabel, quickDates, WEEKDAY_LABELS } from '@/components/pages/listview/calendar';
import { PickerRow } from '@/components/pages/listview/menu-parts';
...
export function StartDateCellEditor({
  task,
  trigger,
}: {
  task: Task;
  ...
  const apply = (value: number | null) => updateTask(task.id, { startDate: value });

  const selected = task.startDate;
