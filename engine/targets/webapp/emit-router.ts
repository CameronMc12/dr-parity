...
export function emitStatefulMain(
  ...
  const content = [
    ...
    '    // A missing/failed mockServiceWorker.js must NEVER block the React',
    '    // render — degrade to real-network passthrough instead of a blank app.',
    '    try {',
    "      const { worker } = await import('./mocks/browser');",
    "      await worker.start({ onUnhandledRequest: 'bypass' });",
    '    } catch (err) {',
    "      console.warn('[dr-parity] MSW worker failed to start:', err);",
    '    }',
    ...socketBoot,
    '  }',
    '',
