/**
 * Emits `src/styles/tokens.css` as a standalone file.
 *
 * Adapts the prototype tokens emitter: hex/rgb values live ONLY here,
 * components reference them via var(--token-name). Adds a small `@media
 * (prefers-reduced-motion: reduce)` helper so animations.ts can branch on it.
 */

import type { DesignTokens } from '../../types/component';

export interface TokensCssResult {
  /** Full contents of tokens.css (with trailing newline). */
  css: string;
}
...
export function buildTokensCss(tokens: DesignTokens): TokensCssResult {
  const lines: string[] = [
    ...
    '/* Hex / rgb values live only in this file. Components must use var(--...). */',
    '',
    ':root {',
  ];
