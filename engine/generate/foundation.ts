...
import type { PageData, FontSpec, SvgEntry, AnimationSpec } from '../types/extraction';
...
function buildGlobalsCss(
  ...
  for (const [varName, varValue] of Object.entries(tokens.cssVariables)) {
    ...
    if (varName.startsWith('--color-') || varName === '--font-sans' || varName === '--font-mono' || varName === '--font-serif') {
      continue;
    }
